...
public class POS_StockManagement extends JPanel implements ActionListener{
	...
	public POS_StockManagement() {
		...
		model.addColumn("재고량");
		model.addColumn("단가");
		jtableStock = new JTable(model);//테이블의 형태로 데이터 출력 뷰(데이터 필요)
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(buttonObject == buttonDB) {
			//DB연결-select 전체검색-vector 담아서-model에 행으로 삽입 --> loadDB(model)
			try {
				loadDB(model);
		...
		}else if(buttonObject == buttonRegister) {//등록
			StockWindow window = new StockWindow(buttonRegister.getText());
		}else if(buttonObject == buttonUpdate) {//수정
			int row = jtableStock.getSelectedRow();
			if(row==-1) {
				JOptionPane.showMessageDialog(null, "셀을 선택하시오", "경고!", JOptionPane.WARNING_MESSAGE);
			}else {
		...
		}else if(buttonObject == buttonDelete) {//삭제
			int row = jtableStock.getSelectedRow();
			if(row==-1) {
				JOptionPane.showMessageDialog(null, "셀을 선택하시오", "경고!", JOptionPane.WARNING_MESSAGE);
			}else {
	...
	public static void loadDB(DefaultTableModel model) throws SQLException {
		...
		for(Item item: itemlist) {
			String item_id = String.valueOf(item.getId());//Integer.toString(), Stirng.valueOf()
